...
- controlled apply for approved runs only; no accept/commit actions.
...
Run detail pages expose post-run analysis/material preparation actions: classify run, run review checks, prepare review, controlled findings submission, controlled arbitration submission, explicit human review decision, and controlled apply for approved runs. They do not run reviewer agents, accept, commit, run Codex, or run pipeline.
...
Record arbitration JSON is available at `/runs/<run_id>/arbitration/new` and `POST /runs/<run_id>/arbitration/record`. The browser submits JSON text, not a file path. The web route validates JSON with the existing `ReviewArbitrationReport` schema, requires existing `REVIEW_FINDINGS.json`, writes a server-generated temporary JSON file under `.web/arbitration_inputs/`, then starts a hidden allowlisted CLI `record-arbitration` job. The UI does not support `--force` overwrite. Arbitration is not approval; review decision is a separate gate, and accept/commit actions are still not exposed.
Record review decision is available at `/runs/<run_id>/review-decision/new` and `POST /runs/<run_id>/review-decision/record`. It records approved/rejected human decisions through a hidden allowlisted CLI `review-run` job. Rejected decisions require feedback text, which is written to a server-generated temporary file under `.web/review_feedback_inputs/`. The UI does not support `--force`, `--from-findings`, or `--force-feedback`. Review decision is not apply, not accept, and not commit.
Apply approved run is available at `/runs/<run_id>/apply/new` and `POST /runs/<run_id>/apply`. It starts a hidden allowlisted CLI `apply-run` job for a human-approved run and requires danger confirmation. Apply modifies the target repository working tree, writes apply reports under `.runs/<run_id>`, does not run `git add`, does not commit, and does not run `accept-run`. The web UI does not pass `--allow-unreviewed`, does not accept a browser-supplied target workspace, and does not use `--target-workspace`. After apply, the operator must inspect `git diff`, run tests, and manually choose what to stage/commit.
...
The local web UI can inspect existing findings at `/runs/<run_id>/findings` and `/runs/<run_id>/findings/<finding_id>`. It can also submit structured JSON findings through `/runs/<run_id>/findings/new`, which validates the JSON schema, writes a server-generated input under `.web/`, and runs the existing CLI `record-findings` through an allowlisted job. The browser never supplies a findings file path, the UI does not pass `--force`, and arbitration/apply/accept/commit actions remain outside the findings submit UI.
The local web UI can inspect existing arbitration at `/runs/<run_id>/arbitration` and `/runs/<run_id>/arbitration/<finding_id>`. This viewer shows stale arbitration status, open blocking findings, missing arbitration coverage, JSON helper examples, and the manual `record-arbitration` CLI command shape. It can also submit structured JSON arbitration through `/runs/<run_id>/arbitration/new`, which validates the JSON schema, requires existing findings, writes a server-generated input under `.web/arbitration_inputs/`, and runs the existing CLI `record-arbitration` through an allowlisted job. The browser never supplies an arbitration file path, the UI does not pass `--force`, and arbitration is not approval.
The local web UI can record explicit human review decisions at `/runs/<run_id>/review-decision/new`. The browser chooses `approved` or `rejected`; rejected decisions require feedback text. Feedback is written to a server-generated input under `.web/review_feedback_inputs/`, and the existing CLI `review-run` runs through an allowlisted job. The UI does not pass `--force`, `--from-findings`, or `--force-feedback`. Review decision is not apply, not accept, and does not commit.
The local web UI can apply an approved run at `/runs/<run_id>/apply/new`. The page requires danger confirmation and starts the existing CLI `apply-run` through a hidden allowlisted job. Apply modifies the target working tree, does not stage files, does not commit, and does not run `accept-run`. The UI does not pass `--allow-unreviewed`, does not pass `--target-workspace`, and never accepts a target workspace path from the browser. After apply, the operator must inspect `git diff`, run tests, then manually stage and commit if desired.
The local web UI can inspect post-apply artifacts at `/runs/<run_id>/apply-report`. This read-only page displays `APPLY_REPORT.json` / `APPLY_REPORT.md`, applied/deleted/skipped files, target workspace, `commit_created=false`, `git_add_performed=false`, and manual next steps. It does not execute `git diff`, tests, `git add`, commit, or `accept-run`; manual commit remains the default final gate.
