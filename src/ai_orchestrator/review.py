...
def write_review_packet(run_dir: Path, *, target_workspace_override: str | None = None) -> Path:
    ...
    lines.extend([
        ...
        "",
        "Deterministic validator approval is not human approval. Review findings or arbitration may still block approval.",
        "Keep the final transition into the target repo human-governed:",
        "",
        "1. Inspect this `REVIEW_PACKET.md` and the generated reports.",
        "2. Record explicit human approval only if the run is acceptable:",
        "",
        "```bash",
        f"./.venv/Scripts/python.exe -m ai_orchestrator.cli review-run {state.run_id} --runs-dir {data.run_dir.parent} --decision approved",
        "```",
        "",
        "3. Apply approved files without staging or committing:",
        "",
        "```bash",
        f"./.venv/Scripts/python.exe -m ai_orchestrator.cli apply-run {state.run_id} --runs-dir {data.run_dir.parent}",
        "```",
        "",
        "4. Inspect the target repo manually:",
        "",
        "```bash",
        "git diff --stat",
        "git diff",
        ...
        "```",
        "",
        "5. Commit manually only after the diff and tests are acceptable:",
        "",
        "```bash",
        "git add <files>",
        "git commit -m \"...\"",
        "```",
        "",
        "`apply-run` refuses non-approved runs, dirty target repos, missing reports, unsafe paths, and generated/runtime files.",
        "",
        "## Advanced delegated commit path",
        "",
        "`accept-run` applies files, stages them, and creates a git commit. Use it only when you explicitly want delegated apply + commit:",
        "",
        "```bash",
